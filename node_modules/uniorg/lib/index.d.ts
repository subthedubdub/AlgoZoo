...
export declare type GreaterElementType = OrgData | Section | PropertyDrawer | Drawer | List | ListItem | QuoteBlock | VerseBlock | CenterBlock | SpecialBlock | FootnoteDefinition | Table;
export declare type ElementType = Headline | Planning | NodeProperty | ListItemTag | CommentBlock | SrcBlock | ExampleBlock | ExportBlock | Keyword | TableRow | Comment | FixedWidth | Clock | LatexEnvironment | HorizontalRule | DiarySexp | Paragraph;
export declare type ObjectType = Link | Bold | Italic | Code | Verbatim | StrikeThrough | Underline | Superscript | Subscript | Text | Timestamp | FootnoteReference | LatexFragment | Entity | TableCell;
export declare type OrgNode = GreaterElementType | ElementType | ObjectType;
...
export interface FootnoteDefinition extends GreaterElement, WithAffiliatedKeywords {
...
export declare type Table = TableOrg | TableTableEl;
...
export interface Timestamp extends Object {
    type: 'timestamp';
    timestampType: 'active' | 'active-range' | 'diary' | 'inactive' | 'inactive-range';
    rawValue: string;
    start: {
        year: number;
        month: number;
...
declare type AffiliatedValue = string | [string, string] | ObjectType[] | [ObjectType[], ObjectType[]];
export declare type AffiliatedKeywords = Record<string, AffiliatedValue | AffiliatedValue[]>;
