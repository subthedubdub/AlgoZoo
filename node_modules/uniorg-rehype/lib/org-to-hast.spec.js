...
const uniorg_parse_1 = __importDefault(require("uniorg-parse"));
...
describe('org/org-to-hast', () => {
    ...
    hastTest('multiple headlines', `* hi
** there
* hello
*** world
`);
    hastTest('complex headline', `* TODO [#A] headline /italic/ title :some:tags:`);
    hastTest('headline with sections', `hello
* hi
section
** hello
another section`);
    hastTest('does not export :noexport: headline', `* hello
some text
* nope :noexport:
not exported text
`);
...
CLOSED: [2019-03-13 Wed 23:48] SCHEDULED: [2019-03-13 Wed] DEADLINE: [2019-03-14 Thu]`);
    hastTest('property drawer', `* headline
:PROPERTIES:
:CREATED: [2019-03-13 Wed 23:57]
:END:`);
    hastTest('custom drawer', `:MYDRAWER:
hello /there/
:END:`);
    hastTest('list', `- hello`);
    hastTest('ordered list', `1. one`);
    hastTest('nested list', `
- hello
  - world
  - blah
- hi
`);
    hastTest('description list', `- term1 :: description 1
- term 2 :: description 2`);
    // See https://github.com/rasendubi/uniorg/issues/15
    hastTest('description list with complex tags', `
...
`);
    ...
    hastTest('escape local link', `[[./path with "spaces".org][path with "spaces"]]`);
    hastTest('escape local link with percent', `[[file:local%2Bpath%2Bwith%2Bpercents.org][local path with percents]]`);
    hastTest('do not double-escape urls', `[[http://localhost:3000/path%2Bwith%2Bpercents][path with percents]]`);
    hastTest('src block', `#+begin_src c
,*a = b;
printf("%d\\n", *a);
#+end_src`);
    hastTest('verse block', `#+BEGIN_VERSE
 ...
 Snow covers Emacs
    ...
    ---AlexSchroeder
#+END_VERSE`);
    hastTest('center block', `#+begin_center
hello
#+end_center`);
    hastTest('comment block', `#+begin_comment
hello
#+end_comment`);
    hastTest('remove common src block offset', `
  #+begin_src
    hello
      world
  #+end_src`);
    hastTest('example block', `#+begin_example
example
#+end_example`);
    hastTest('export block', `#+begin_export
export
#+end_export`);
    hastTest('export html block', `
#+begin_export html
<abbr title="World Health Organization">WHO</abbr> was founded in 1948.
#+end_export
`);
    hastTest('export html keyword', `hello
#+HTML: <h1>html tag</h1>`);
    hastTest('special block', `#+begin_blah
hello
#+end_blah`);
    hastTest('html5 fancy block', `#+begin_aside
hello
#+end_aside`);
    hastTest('blockquote', `#+begin_quote
hello, world!
#+end_quote`);
    hastTest('keywords', `#+TITLE: blah`);
    hastTest('emphasis', `/Consider/ ~t*h*e~ *following* =example= +strike+ _under_`);
...
| value1 | value2 |
`);
    hastTest('table without header', `
| value1 | value2 |
| value3 | value4 |
`);
    hastTest('table, multiple body', `
| head1  | head2 |
|--------+-------|
| value1 | value2 |
| value3 | value4 |
...
there`);
    ...
    hastTest('clock', `CLOCK: [2020-12-22 Tue 09:07]--[2020-12-22 Tue 11:10] =>  2:03`);
    hastTest(`latex environment`, `\\begin{hello}
some text
\\end{hello}`);
